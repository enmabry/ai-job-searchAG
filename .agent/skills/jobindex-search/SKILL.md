...
context: fork
allowed-tools: Bash(bun run .agent/skills/jobindex-search/cli/src/cli.ts *)
---
...
```bash
bun run .agent/skills/jobindex-search/cli/src/cli.ts search [flags]
```
...
```bash
bun run .agent/skills/jobindex-search/cli/src/cli.ts detail <id> [--format json|plain]
```
...
```bash
bun run .agent/skills/jobindex-search/cli/src/cli.ts search \
  --query python \
  --jobage 7 \
  --sort date \
  --format table
...
```bash
bun run .agent/skills/jobindex-search/cli/src/cli.ts detail h1647303 --format plain
```
