...
Checks:
1. .agent/settings.json — every permissions.allow entry must be in the exact
...
3. .agent/**/package.json — no npm/bun lifecycle scripts (preinstall,
...
def check_package_manifests() -> None:
    manifests = [
        p for p in ROOT.glob(".agent/**/package.json") if "node_modules" not in p.parts
    ]
    if not manifests:
