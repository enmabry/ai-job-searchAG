---
name: add-portal
description: Add-portal command.
---
...
- If `$ARGUMENTS` contains `--list`: use Glob with `.agent/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
...
2. **Skill name** - kebab-case, suffixed `-search` (e.g. `seek-search`, `stepstone-search`). Must not collide with an existing folder in `.agent/skills/`.
...
**Canonical reference:** read `.agent/skills/linkedin-search/` before generating - it is the zero-dependency worked example of this exact structure. Copy its architecture, not its LinkedIn-specific parsing.

Create `.agent/skills/<name>/` with:
...
1. Install dev types and typecheck:
   ```bash
   cd .agent/skills/<name>/cli && bun install && bun run typecheck
   ```
...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes, add the portal's site to the relevant query categories in `.agent/skills/job-scraper/search-queries.md` (site-specific queries, like the existing `jobindex.dk` entries) so `/scrape` includes it.
2. Remind the user to add the install line for their own records if they maintain a fork README:
   ```bash
   cd .agent/skills/<name>/cli && bun install && cd ../../../..
   ```
...
>
> - Files: `.agent/skills/<name>/` (SKILL.md, url-reference.md, CLI with tests)
...
>
> Try it: `bun run .agent/skills/<name>/cli/src/cli.ts search -q "<test query>" --format table`
>
